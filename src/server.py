            ...
            for i in range(max_seq):
                try:
                    ...
                    if not received_segment.is_valid_checksum():
                        print(
                            f"[!] [Client {client_address}] [Error] Invalid checksum received. Resending segments..."
                        )
                        continue

                    if (
                        not message_info.ip == client_address[0]
                        or not message_info.port == client_address[1]
                    ):
                        print(
                            f"[!] [Client {client_address}] [Error] Invalid client received. Resending segments..."
                        )
                        continue
                    ...
                    if (
                        ...
                        sequence_base += 1
                        print(
                            f"[!] [Client {client_address}] [ACK] ACK number {received_segment.acknowledgment_number} received, new sequence base = {sequence_base}"
                        )
                    elif (
                        received_segment.is_ack()
                        and received_segment.acknowledgment_number > sequence_base
                        ...
                        print(
                            f"[!] [Client {client_address}] [ACK] ACK number {received_segment.acknowledgment_number} received < sequence base {sequence_base}"
                        )
