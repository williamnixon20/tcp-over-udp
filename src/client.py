        ...
        while True:
            try:
                ...
                if received_segment.is_fin():
                    print("[Segment SEQ={}] Received FIN. Acking and closing connection.".format(received_segment.sequence_number))
                    ack_segment = Segment.ack(expected_sequence_number, received_segment.sequence_number)
                    self.connection.send(message_info.ip, message_info.port, ack_segment)
                    break
                    ...
                    print("[Segment SEQ={}] Ignoring, invalid segment received. Expected SEQ={}".format(received_segment.sequence_number, expected_sequence_number))
                    
            except Exception as e:
                print("[!] Timeout waiting for segment. Retrying...")
                ack_segment = Segment.ack(expected_sequence_number-1, expected_sequence_number-1)
                self.connection.send(message_info.ip, message_info.port, ack_segment)
